...
import com.automatica.AXCPT.databinding.ActivitySurtidoProdSeleccionPartidaBinding;
...
public class SurtidoProdSeleccionPartida extends AppCompatActivity implements TableViewDataConfigurator.TableClickInterface, frgmnt_taskbar_AXC.interfazTaskbar{
    ...
    popUpGenerico pop = new popUpGenerico(SurtidoProdSeleccionPartida.this);
    ActivitySurtidoProdSeleccionPartidaBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivitySurtidoProdSeleccionPartidaBinding.inflate(getLayoutInflater());
    ...
    private void agregarListener(){
            ...
            binding.edtxDocumento.setOnKeyListener(new View.OnKeyListener()
            {
                ...
                public boolean onKey(View v, int keyCode, KeyEvent event)
                {
                    if((event.getAction()==KeyEvent.ACTION_DOWN)&&(keyCode==KeyEvent.KEYCODE_ENTER))
                    {
                        if(binding.edtxDocumento.getText().toString().equals(""))
                        {
                            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxDocumento ,getString(R.string.error_ingrese_pedido) ,"false" , true, true);
                            h.post(new Runnable()
                            {
                                @Override
                                public void run()
                        ...
                        new SurtidoProdSeleccionPartida.SegundoPlano("Tabla").execute();

                        new esconderTeclado(SurtidoProdSeleccionPartida.this);
            ...
            binding.edtxCarrito.setOnKeyListener(new View.OnKeyListener()
            {
                ...
                public boolean onKey(View v, int keyCode, KeyEvent event)
                {
                    if((event.getAction()==KeyEvent.ACTION_DOWN)&&(keyCode==KeyEvent.KEYCODE_ENTER))
                    {
                        if(binding.edtxCarrito.getText().toString().equals(""))
                        {
                            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxCarrito ,getString(R.string.error_ingrese_ubicacion) ,"false" , true, true);
                            h.post(new Runnable()
                            {
                                @Override
                                public void run()
        ...
        {
            e.printStackTrace();
            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,null ,e.getMessage() ,false , true, true);
    ...
    private void validacionFinal(){
        ...
        if (binding.edtxCarrito.getText().toString().equals("")){
            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxCarrito ,"Para surtir debe de ingresar un código de carrito" , false, true,true );
            return;
        }
        ...
        if(binding.edtxDocumento.getText().toString().equals(""))
        {
            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxDocumento,getString(R.string.error_seleccionar_registro),false,true,false);
           // reiniciarDatos();
            return;
        }
                ...
                new PopUpMenuAXC.ContextMenuListener()
                {
                    ...
                    public void listenerItem(MenuItem item)
                    {
                        ...
                        }else
                        {
                            ...
                            {
                                new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxCarrito ,"Consulte el carrito de nuevo." , false, true,true );
                                return;
                            }
                        }
                        ...
                        switch (item.getItemId())
                        {
                            case R.id.Empaque:
                                if (tipoSurtido.equals("PICKING") || tipoSurtido.equals("TODO"))
                                    intent = new Intent(contexto, SurtidoProdEmpaque.class);
                                else{
                            ...
                            case R.id.Piezas:
                                if (tipoSurtido.equals("PICKING") || tipoSurtido.equals("TODO"))
                                    intent = new Intent(contexto, SurtidoProdPiezas.class);
                                else{
                                    new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,binding.edtxCarrito ,"Opción de surtido no válida, intente surtir por ["+tipoSurtido+"]" , false, true,true );
                                    return;
                                }
    ...
    public void onBackPressed() {
        ...
        Intent intent = new Intent(SurtidoProdSeleccionPartida.this, SurtidoProdSeleccionOrden.class);
    ...
    private class SegundoPlano extends AsyncTask<Void, Void, Void> {
        ...
        cAccesoADatos_RegistroPT adEmb = new cAccesoADatos_RegistroPT(SurtidoProdSeleccionPartida.this);
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                switch (Tarea) {
                    case "LlenarTabla":
                        dao = adEmb.cad_ConsultaPedidoSurtidoProd(binding.edtxDocumento.getText().toString());
                        break;

                    case"ConsultaCarrito":
                        dao = adEmb.cad_ConsultaCarritoSurtidoProd(binding.edtxDocumento.getText().toString() ,binding.edtxCarrito.getText().toString());
                        break;
                }
            } catch (Exception e) {
            ...
            try {
                if (dao.iscEstado()) {
                    switch (Tarea) {
                        case "LlenarTabla":
                            if (ConfigTabla_Totales == null) {
                                ConfigTabla_Totales =  new TableViewDataConfigurator( 7, "SURTIDA","LIBERADA TOTAL","10",tabla, dao,SurtidoProdSeleccionPartida.this);
