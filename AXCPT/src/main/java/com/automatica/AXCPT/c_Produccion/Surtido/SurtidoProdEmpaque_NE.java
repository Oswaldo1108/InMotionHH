...
public class SurtidoProdEmpaque_NE extends AppCompatActivity  implements TableViewDataConfigurator.TableClickInterface, frgmnt_taskbar_AXC.interfazTaskbar {
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        new cambiaColorStatusBar(contexto, R.color.MoradoStd, SurtidoProdEmpaque_NE.this);
        SacaExtrasIntent();
        declararVariables();
        agregaListeners();
    ...
    private void Recarga()
    {
        if(!binding.txtvPedido.getText().toString().equals("-")) {
            ...
            if (binding.clRegistroEmpaque.getVisibility() == View.VISIBLE)
            {
                ...
                new SurtidoProdEmpaque_NE.SegundoPlano("ConsultaPedidoSurtido").execute("@");

            } else if (binding.clTablaRegistro.getVisibility() == View.VISIBLE)
            {
                new SurtidoProdEmpaque_NE.SegundoPlano("ConsultaPalletAbierto").execute();
            }
        }
    }
...
//                        }
                        ...
                        new SurtidoProdEmpaque_NE.SegundoPlano("RegistrarEmpaque").execute();
                    }else
                    {
                        handler.post(new Runnable()
                        {
                    ...
                    new esconderTeclado(SurtidoProdEmpaque_NE.this);
                    return true;
                }
                return false;
            }
        ...
        edtx_Empaque.setOnKeyListener(new View.OnKeyListener()
        {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event)
            {
                if((event.getAction()==KeyEvent.ACTION_DOWN)&&(keyCode==KeyEvent.KEYCODE_ENTER))
                {
                    if(!edtx_Empaque.getText().toString().equals(""))
                    {
                        new SurtidoProdEmpaque_NE.SegundoPlano("ConsultaEmpaque").execute();
                    }else
                    {
                        new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,getCurrentFocus() ,getString(R.string.error_ingrese_empaque) ,"false" ,true , true);
        ...
        sp_partidas.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener()
        {
            ...
            {
                new SurtidoProdEmpaque_NE.SegundoPlano("ConsultaPedidoDet").execute(((Constructor_Dato) sp_partidas.getSelectedItem()).getDato());
            }
            ...
            if (progressBarHelper.ispBarActiva())
            {
                ...
                if ((id == R.id.CambiarVista))
                {
                    ...
                    } else if (cl_TablaRegistro.getVisibility() == View.VISIBLE)
                    {
                        ...
                        new SurtidoProdEmpaque_NE.SegundoPlano("ConsultaPedidoDet").execute("@");
    ...
    public void BotonDerecha()
    {
        ...
        new CreaDialogos(getString(R.string.pregunta_cierre_pallet),
                new DialogInterface.OnClickListener()
                {
                    ...
                    {
                        new SurtidoProdEmpaque_NE.SegundoPlano("CerrarTarima").execute();
                    }
                },null,contexto);
    }
    ...
    private class SegundoPlano extends AsyncTask<String,Void,Void>
    {
        ...
        cAccesoADatos_RegistroPT cadEmb = new cAccesoADatos_RegistroPT(SurtidoProdEmpaque_NE.this);
        ...
        protected Void doInBackground(String... params)
        {
            try
            {
                switch (tarea)
                {
                    ...
                    case "SugiereEmpaque":
                        dao = cadEmb.cad_ConsultaEmpaqueSugeridoProd_NE(Pedido,Partida);
                        break;

                    case "ConsultaEmpaque":
                        dao = cadEmb.cad_ConsultaPalletSurtidoProd_NE(Pedido,Partida,edtx_Empaque.getText().toString());
                        break;

                    case "RegistrarEmpaque":
                        dao = cadEmb.cad_RegistroEmpaqueSurtidoProd_NE(Pedido,Partida,edtx_Empaque.getText().toString(),edtx_ConfirmarEmpaque.getText().toString(),txtv_PalletAbierto.getText().toString());
                        break;

                    case "ConsultaPalletAbierto":

                        dao = cadEmb.cad_ConsultaTarimaConsolidadaProd(Pedido,txtv_PalletAbierto.getText().toString());

                        break;

                    case "CerrarTarima":
                        dao = cadEmb.cad_CierraPalletSurtidoProd(txtv_PalletAbierto.getText().toString());
                        break;
                ...
                if (dao.iscEstado())
                {
                    switch (tarea)
                    {
                        case"ConsultaPedidoSurtido":
                            ...
                            {
                                sp_partidas.setAdapter(new CustomArrayAdapter(SurtidoProdEmpaque_NE.this,
                        ...
                        case "ConsultaPedidoDet":
                            ...
                            new SurtidoProdEmpaque_NE.SegundoPlano("SugiereEmpaque").execute();
                            break;
                            ...
                            if(dao.getcTablaUnica()!=null)
                            {
                                if(ConfigTabla == null)
                                {
                                    ConfigTabla =  new TableViewDataConfigurator(2,"Listo",
                                            "En proceso","-",tabla_Salidas, dao, SurtidoProdEmpaque_NE.this);
                                }else
                                {
