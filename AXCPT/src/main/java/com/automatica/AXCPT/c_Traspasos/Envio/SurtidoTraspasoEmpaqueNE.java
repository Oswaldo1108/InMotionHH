...
import com.automatica.AXCPT.databinding.ActivitySurtidoProdEmpaqueNeBinding;
...
public class SurtidoTraspasoEmpaqueNE extends AppCompatActivity  implements TableViewDataConfigurator.TableClickInterface, frgmnt_taskbar_AXC.interfazTaskbar {
    ...
    private ActivitySurtidoProdEmpaqueNeBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        binding = ActivitySurtidoProdEmpaqueNeBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        new cambiaColorStatusBar(contexto, R.color.MoradoStd, SurtidoTraspasoEmpaqueNE.this);
        SacaExtrasIntent();
        declararVariables();
        agregaListeners();
    ...
    {
        if(!binding.txtvPedido.getText().toString().equals("-")) {


            if (binding.clRegistroEmpaque.getVisibility() == View.VISIBLE)
            {
            ...
            {
                new SurtidoTraspasoEmpaqueNE.SegundoPlano("ConsultaPalletAbierto").execute();
            }
        }
    }
...
//                        }
                        ...
                        new SurtidoTraspasoEmpaqueNE.SegundoPlano("RegistrarEmpaque").execute();
                    }else
                    {
                        handler.post(new Runnable()
                        {
                    ...
                    new esconderTeclado(SurtidoTraspasoEmpaqueNE.this);
                    return true;
                }
                return false;
            }
        ...
        edtx_Empaque.setOnKeyListener(new View.OnKeyListener()
        {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event)
            {
                if((event.getAction()==KeyEvent.ACTION_DOWN)&&(keyCode==KeyEvent.KEYCODE_ENTER))
                {
                    if(!edtx_Empaque.getText().toString().equals(""))
                    {
                        new SurtidoTraspasoEmpaqueNE.SegundoPlano("ConsultaEmpaque").execute();
                    }else
                    {
                        new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,getCurrentFocus() ,getString(R.string.error_ingrese_empaque) ,"false" ,true , true);
        ...
        sp_partidas.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener()
        {
            ...
            {
                new SurtidoTraspasoEmpaqueNE.SegundoPlano("ConsultaPedidoDet").execute(((Constructor_Dato) sp_partidas.getSelectedItem()).getDato());
            }
        ...
        txtv_SugPallet.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (txtv_SugPallet.getText().toString().equals("-")|| TextUtils.isEmpty(txtv_SugPallet.getText())){
                    new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,getCurrentFocus(),"Consulta un pallet para continuar",false,true,true);
                    return false;
                }
        ...
        txtv_SugPosicion.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (txtv_SugPosicion.getText().toString().equals("-")|| TextUtils.isEmpty(txtv_SugPosicion.getText())){
                    new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,getCurrentFocus(),"Consulta una posición para continuar",false,true,true);
                    return false;
                }
            ...
            if (progressBarHelper.ispBarActiva())
            {
                ...
                if ((id == R.id.CambiarVista))
                {
                    ...
                    } else if (cl_TablaRegistro.getVisibility() == View.VISIBLE)
                    {
                        ...
                        new SurtidoTraspasoEmpaqueNE.SegundoPlano("ConsultaPedidoDet").execute("@");
    ...
    public void BotonDerecha()
    {
        ...
        {
            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto,null, getString(R.string.error_cerrar_pallet_sin_empaques) , "false",true , true);
            return;
        }
        ...
        new CreaDialogos(getString(R.string.pregunta_cierre_pallet),
                new DialogInterface.OnClickListener()
                {
                    ...
                    {
                        new SurtidoTraspasoEmpaqueNE.SegundoPlano("CerrarTarima").execute();
                    }
                },null,contexto);
    }
    ...
    private class SegundoPlano extends AsyncTask<String,Void,Void>
    {
        ...
        cAccesoADatos_Almacen cadEmb = new cAccesoADatos_Almacen(SurtidoTraspasoEmpaqueNE.this);
            ...
            try
            {
                switch (tarea)
                {
                    case"ConsultaPedidoSurtido":
                        dao = cadEmb.cad_ListarPartidasTrasSpinner(Pedido);
                        break;
                    case "ConsultaPedidoDet":
                        ...
                        dao = cadEmb.cad_ConsultaSurtidoTrasDetPartida(Pedido,Partida);

                        break;

                    case "SugiereEmpaque":
                        dao = cadEmb.cad_ConsultaEmpaqueSugeridoTras_NE(Pedido,Partida);
                        break;

                    case "ConsultaEmpaque":
                        dao = cadEmb.cad_ConsultaPalletSurtidoTras_NE(Pedido,Partida,edtx_Empaque.getText().toString());
                        break;

                    case "RegistrarEmpaque":
                        dao = cadEmb.cad_RegistroEmpaqueSurtidoTras_NE(Pedido,Partida,edtx_Empaque.getText().toString(),edtx_ConfirmarEmpaque.getText().toString(),txtv_PalletAbierto.getText().toString());
                        break;

                    case "ConsultaPalletAbierto":

                        dao = cadEmb.cad_ConsultaTarimaConsolidadaTras(Pedido,txtv_PalletAbierto.getText().toString());

                        break;

                    case "CerrarTarima":
                        dao = cadEmb.cad_CierraPalletSurtidoTras(txtv_PalletAbierto.getText().toString());
                        break;
                ...
                if (dao.iscEstado())
                {
                    switch (tarea)
                    {
                        case"ConsultaPedidoSurtido":
                            ...
                            {
                                sp_partidas.setAdapter(new CustomArrayAdapter(SurtidoTraspasoEmpaqueNE.this,
                        ...
                        case "ConsultaPedidoDet":
                            ...
                            new SurtidoTraspasoEmpaqueNE.SegundoPlano("SugiereEmpaque").execute();
                            break;
                            ...
                            if(dao.getcTablaUnica()!=null)
                            {
                                if(ConfigTabla == null)
                                {
                                    ConfigTabla =  new TableViewDataConfigurator(2,"Listo",
                                            "En proceso","-",tabla_Salidas, dao, SurtidoTraspasoEmpaqueNE.this);
                                }else
                                {
                        ...
                        case "CerrarTarima":
                            new com.automatica.AXCPT.Servicios.popUpGenerico(contexto, getCurrentFocus(), "Tarima cerrada [" + RegEmpPalletAbierto + "] con éxito.", dao.iscEstado(), true, true);
