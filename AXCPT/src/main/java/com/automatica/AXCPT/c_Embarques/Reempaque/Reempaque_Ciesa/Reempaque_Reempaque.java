...
public class Reempaque_Reempaque extends AppCompatActivity implements frgmnt_taskbar_AXC.interfazTaskbar, frgmnt_Seleccion_Producto.OnFragmentInteractionListener, frgmnt_SKU_Conteo.OnFragmentInteractionListener{
    ...
    private ActivityReempaqueReempaqueBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        try
        {
            ...
            binding = ActivityReempaqueReempaqueBinding.inflate(getLayoutInflater());
    ...
    private void InicializarVariables()
    {
        try
        {
            ...
            activityHelpers.AgregarMenus(Reempaque_Reempaque.this, R.id.Pantalla_principal, false);
            ...
            tblcnf_Carrito = TableViewDataConfigurator.newInstance(binding.customtableview2.tableViewOC, null, Reempaque_Reempaque.this, new TableViewDataConfigurator.TableClickInterface()
            {
                @Override
                public void onTableHeaderClick(int columnIndex, String Header, String IdentificadorTabla)
                {
            ...
            tblcnf_ReempaqueAbierto = TableViewDataConfigurator.newInstance(binding.customtableview3.tableViewOC, null, Reempaque_Reempaque.this, new TableViewDataConfigurator.TableClickInterface()
            {
                @Override
                public void onTableHeaderClick(int columnIndex, String Header, String IdentificadorTabla)
                {
                ...
                public boolean onEditorAction(TextView v, int actionId, KeyEvent event)
                {
                    ...
                    try
                    {
                       ...
                       /* if (!tblcnf_Carrito.renglonEstaSelec())
                        ...
                        if (TextUtils.isEmpty(binding.txtvConsolidado.getText()))
                        {
                            new popUpGenerico(contexto, getCurrentFocus(), "Seleccione un documento", false, true, true);
                            binding.txtvConsolidado.requestFocus();
                            return false;
            ...
            try
            {
                switch (Tarea)
                {
                    case "ConsultarDocumento":
                        dao = ca.cConsultaListaConsolidadosReempaque(binding.txtvConsolidado.getText().toString());
                        break;
                    case "ConsultaTabla":
                        dao = ca.cad_ConsultaPedidoSurtidoReempaque(spinnerDocumentos.getSelectedItem().toString());
                        break;
                    case "ConsultaCarrito":
                        dao = ca.cad_ConsultaCarritoEmpaques(binding.txtvConsolidado.getText().toString(),binding.edtxCarrito.getText().toString());
                        break;
                    case "ConsultaPalletAbierto":
                        dao = ca.cCreaPalletReempaque(binding.txtvConsolidado.getText().toString());
                        break;
                    case "ConsultaEmpaquesPalletCons":
                        dao = ca.cConsultaPalletCons(binding.txtvPallet.getText().toString());
                        break;
                    ...
                    case "RegistraEmpaqueCons":
                        dao = ca.cRegistraReempaqueCons(binding.txtvConsolidado.getText().toString(),binding.txtvPallet.getText().toString(),
                ...
                if (dao.iscEstado())
                {
                    switch (Tarea)
                    {
                        ...
                        case "ConsultaCarrito":
                            ...
                            new esconderTeclado(Reempaque_Reempaque.this);
